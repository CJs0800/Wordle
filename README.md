# Wordle

[![Live Demo](https://img.shields.io/badge/Live-Demo-brightgreen?logo=github)](https://CJs0800.github.io/Wordle/)
[![Vue](https://img.shields.io/badge/Vue-3.x-42b883?logo=vue.js)](https://vuejs.org/)
[![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-f7df1e?logo=javascript&logoColor=black)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
[![Vite](https://img.shields.io/badge/Vite-Build%20Tool-646cff?logo=vite)](https://vitejs.dev/)
[![Platform](https://img.shields.io/badge/Platform-Web-green)](#)
[![Responsive](https://img.shields.io/badge/UI-Responsive-green)](#)
[![Version](https://img.shields.io/badge/Version-1.0.1-success)](https://github.com/CJs0800/Wordle/releases/tag/1.0.1)
[![Game](https://img.shields.io/badge/Game-Wordle%20Clone-orange)](https://fr.wikipedia.org/wiki/Wordle)
[![License](https://img.shields.io/badge/License-All%20Rights%20Reserved-red)](https://en.wikipedia.org/wiki/All_rights_reserved)

---


<details>
  <summary><h2>English README</h2></summary>

> **⮕ Live Demo :** https://CJs0800.github.io/Wordle/
> --

This project was developed in 2024 as part of a group university project under a (very) limited time constraint.  
The objective was to deliver a functional and user-friendly solution as a team while applying the skills acquired in UI/UX design and Frontend development.

---

## Features

- Wordle-style gameplay 
- Color feedback system (correct / present / absent letters)
- Interactive on-screen keyboard
- Word validation and guess checking
- Dynamic grid interface
- Built with Vue 3 and Vite

---

## Supported Platforms

-   ✅ Web ([ GitHub Pages ](https://CJs0800.github.io/Wordle))

---

## Project setup
```
npm install
```

### Compiles and hot-reloads for development
```
npm run serve
```

### Compiles and minifies for production
```
npm run build
```

### Run tests
```
npm run test
```

---

## Deployment

> Live version:\
> 👉 https://CJs0800.github.io/Wordle/

---

## Changelog

> All notable changes to this project will be documented in this file.

#### [[1.0.1]](https://github.com/CJs0800/Wordle/releases/tag/1.0.1) - README Update
#### [[1.0.0]](https://github.com/CJs0800/Wordle/releases/tag/1.0.0) - Runnable Version

---

## About the Developer

Developed by :
- **Corentin JÉRÉ**
[![GitHub](https://img.shields.io/badge/GitHub-CJs0800-yellow?logo=github)](https://github.com/CJs0800)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Corentin_JERE-0A66C2?logo=linkedin&logoColor=blue)](https://linkedin.com/in/corentin-jere/)
[![Email](https://img.shields.io/badge/Email-corentinjere@gmail.com-red?logo=gmail&logoColor=white)](mailto:corentinjere@gmail.com)
- **Alex LECOMTE**
[![GitHub](https://img.shields.io/badge/GitHub-alexLcmt-yellow?logo=github)](https://github.com/alexLcmt)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Alex_LECOMTE-0A66C2?logo=linkedin&logoColor=blue)](https://linkedin.com/in/alex-lecomte-3a6503296/)
[![Email](https://img.shields.io/badge/Email-alexlecomte144@gmail.com-red?logo=gmail&logoColor=white)](mailto:alexlecomte144@gmail.com)

</details>

---


<details open>
  <summary><h2>README Français</h2></summary>

> **⮕ Démo en ligne :** https://CJs0800.github.io/Wordle/
> --

Ce projet a été développé en 2024 lors d'un projet universitaire en groupe dans un contexte de temps (très) limité.  
L'objectif était de réaliser une solution fonctionnelle et ergonomique en équipe en mettant en pratique les compétences acquises en UI/UX et Application Frontale. 

---

## Plateformes supportées

- ✅ Web ([ GitHub Pages ](https://CJs0800.github.io/Wordle))

---

## Fonctionnalités

- Gameplay type Wordle
- Système de couleurs de validation des propositions
- Clavier virtuel interactif
- Validation et vérification des mots
- Interface dynamique
- Application web rapide développée avec Vue 3 et Vite

---

## Installation

```
npm install
```

### Compile et recharge automatiquement l'application pendant le développement
```
npm run serve
```

### Compile et optimise l'application pour la production
```
npm run build
```

### Exécuter les tests
```
npm run test
```

---

## Déploiement

> Version en ligne :  
> 👉 https://CJs0800.github.io/Wordle/

---

## Changelog

> Toutes les modifications importantes apportées à ce projet seront documentées dans ce fichier.

#### [[1.0.1]](https://github.com/CJs0800/Wordle/releases/tag/1.0.1) - README Update
#### [[1.0.0]](https://github.com/CJs0800/Wordle/releases/tag/1.0.0) - Runnable Version

---

## À propos du développeur

Développé par : 
- **Corentin JÉRÉ** 
[![GitHub](https://img.shields.io/badge/GitHub-CJs0800-yellow?logo=github)](https://github.com/CJs0800)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Corentin_JERE-0A66C2?logo=linkedin&logoColor=blue)](https://linkedin.com/in/corentin-jere/)
[![Email](https://img.shields.io/badge/Email-corentinjere@gmail.com-red?logo=gmail&logoColor=white)](mailto:corentinjere@gmail.com)
- **Alex LECOMTE** 
[![GitHub](https://img.shields.io/badge/GitHub-alexLcmt-yellow?logo=github)](https://github.com/alexLcmt)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Alex_LECOMTE-0A66C2?logo=linkedin&logoColor=blue)](https://linkedin.com/in/alex-lecomte-3a6503296/)
[![Email](https://img.shields.io/badge/Email-alexlecomte144@gmail.com-red?logo=gmail&logoColor=white)](mailto:alexlecomte144@gmail.com)

</details>

---


## 📄 License

© 2026 CJs0800. All Rights Reserved.

This project is publicly accessible for viewing purposes only.  
No part of this code may be copied, modified, distributed, or reused without explicit permission from the author.

---

[![Vue](https://img.shields.io/badge/Vue-3.x-42b883?logo=vue.js)](https://vuejs.org/)
[![JavaScript](https://img.shields.io/badge/JavaScript-ES6+-f7df1e?logo=javascript&logoColor=black)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
[![Vite](https://img.shields.io/badge/Vite-Build%20Tool-646cff?logo=vite)](https://vitejs.dev/)
![Responsive UI](https://img.shields.io/badge/Responsive-UI-green)
![Game Logic](https://img.shields.io/badge/Game%20Logic-Structured-success)
![Validation](https://img.shields.io/badge/Input-Validation-red)
![Git](https://img.shields.io/badge/Git-Version%20Control-orange?logo=git)
![GitHub Pages](https://img.shields.io/badge/Deployment-GitHub%20Pages-black?logo=github)
